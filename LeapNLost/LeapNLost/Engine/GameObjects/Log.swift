...
class Log : GameObject{
    ...
    init(pos: Vector3, speed: Float){
        self.speed = speed;
        self.glow = PointLight(color: Vector3(0.8, 0.2, 0.2), ambientIntensity: 1, diffuseIntensity: 1, specularIntensity: 1, position: Vector3(), constant: 2.0, linear: 1.5, quadratic: 1);
        super.init(ModelCacheManager.loadModel(withMeshName: "log", withTextureName: "log.png", saveToCache: true)!);
